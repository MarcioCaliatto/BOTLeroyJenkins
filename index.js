...
async function music_message(message, mapKey) {
  ...
  for (let mess of messes) {
    ...
    if (args[0] == _CMD_PLAY && args.length) {
        ...
        if (isYoutube(qry) && isYoutubePlaylist(qry)) {
          ...
          try {
            ...
            message.channel.send(
              "Failed to find video for (try again): " + qry
            );
          }
        }
      }
      ...
      const chunks = message_chunking(
        getQueueString(mapKey),
        DISCORD_MSG_LIMIT
      );
...
function unFavorite(qry, mapKey, cbok, cberr) {
  let val = guildMap.get(mapKey);
  if (!qry || !qry.length) cberr("Invalid query.");
  else {
    if (!(mapKey in GUILD_FAVORITES)) {
      cberr("No favorites.");
    } else {
      if (GUILD_FAVORITES[mapKey].includes(qry)) {
        GUILD_FAVORITES[mapKey] = GUILD_FAVORITES[mapKey].filter(
          (e) => e !== qry
        );
        cbok();
      } else {
...
function isYoutubePlaylist(str) {
  return (
    str.toLowerCase().indexOf("?list=") > -1 ||
    str.toLowerCase().indexOf("&list=") > -1
  );
}
...
async function getYoutubeVideoData(str, isretry = false) {
  try {
    ...
    let qry = str;
    if (isYoutube(str)) qry = getYoutubeID(str);
